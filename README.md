...
3. POST http://localhost/api/v2/applications/ - сохранить заявку в БД.
...
Обязательные поля такие же, как в запросе на сохранение.
Если необязательные поляотстутсвуют, то их значения в БД не изменятся после выполнения запроса.
Пример запроса (payload)
```JSON
{
    "application": {
...
```
5. POST http://localhost/api/users/id(целое число)/subscribe/ - подписаться на пользователя.
...
11. GET http://localhost/api/recipes/download_shopping_cart/ - скачать список покупок в виде PDF или TXT файла.](http://localhost:8000/api/v2/citizenships/)http://localhost:8000/api/v2/citizenships/](http://localhost:8000/api/v2/citizenships/)http://localhost:8000/api/v2/citizenships/
